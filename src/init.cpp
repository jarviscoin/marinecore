    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("MarineCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  marinecoin [options]                     " + "\n" +
                  "  marinecoin [options] <command> [params]  " + _("Send command to -server or marinecoin") + "\n" +
                  "  marinecoin [options] help                " + _("List commands") + "\n" +
                  "  marinecoin [options] help <command>      " + _("Get help for a command") + "\n";
...
{
    uiInterface.ThreadSafeMessageBox(str, _("MarineCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("MarineCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: marinecoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: marinecoin.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 54596 or testnet: 45884)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 54595)") + "\n" +
    ...
    strUsage += string() +
        _("\nSSL options: (see the Marinecore Wiki for SSL setup instructions)") + "\n" +
...
//    }
    ...
    if (mapArgs.count("-paytxfee"))
    {
        ...
        if (nTransactionFee > 0.0025 * COIN)
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  MarineCoin is probably already running."), GetDataDir().string().c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "MarineCoin server starting\n");
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of MarineCoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart MarineCoin to complete") << "\n";
